...
## 섹션8. 라이프사이클(LifeCycle) = 컴포넌트 생애주기

### 라이프사이클이란?

- 생애주기 = 탄생부터 죽음까지를 단계별로 나눈 것 (인간: 영유아→아동기→…→노년기)
- 리액트 컴포넌트도 인간처럼 라이프사이클을 가진다 (3단계)

### ⭐ 컴포넌트 3단계

| 단계    | 뜻                                   | 비유    |
| ------- | ------------------------------------ | ------- |
| Mount   | 화면에 "처음" 렌더링 되는 순간       | 탄생 🐣 |
| Update  | 다시 렌더링(리렌더) 되는 순간        | 변화 🔄 |
| Unmount | 화면에서 사라지는 (렌더링 제외) 순간 | 죽음 💀 |

- "A 마운트됐다" = A가 처음 렌더링됐다
- "A 업데이트됐다" = A가 리렌더일됐다
- "A 언마운트됐다" = A가 화면에서 사라졌다

### 🔗 이미 아는 거랑 연결

- Update = 리렌더링 → 리렌더 3원인(①내 state 변경 ②받는 props 변경 ③부모 리렌더)이 일어나는 순간
- Mount = 첫 렌더 -> 화면에 처음 그려지는 (print) 순간
  => 새 개념 아님 ! 아는 "렌더링" 단계 이름표 붙인 것

### ⭐ 왜 배우나 = 라이프사이클 "제어"

- 각 단계(타이밍)에 원하는 작업을 실행시키기
- Mount 때 → 서버에 데이터 요청해서 불러오기 (★ 비동기 트랙과 연결!)
- Update 때 → 바뀐 state 콘솔 출력 / 저장
- Unmount 때 → 쓰던 메모리·타이머 정리(cleanup)

### 도구 예고

- 라이프사이클 제어를 쉽게 해주는 훅 = 다음 시간 useEffect

### 💡 한 줄 정리

컴포넌트도 태어나고(Mount)→ 바뀌고(Update)→ 죽는다(Unmount). 도구는 useEffect.

## 섹션8. useEffect — 사이드 이펙트 제어 훅

### Side Effect란?

- 직역=부작용 / React에선 "어떤 동작에 따라 파생되는 부수적 효과"
- 비유: 과식(동작) → 살찐다(side effect)
- 예: 마운트 때 콘솔 출력 / 업데이트 때 바뀐 값 출력 / 언마운트 때 메모리 정리
- useEffect = 이런 side effect를 만들고 "제어"하는 훅 (라이프사이클 제어)

### ⭐ 기본 사용법

    useEffect(() => {실행할 코드}, [ 의존성 ]);
    //        ┗1번 인수: 콜백함수┛  ┗2번 인수: 의존성 배열┛

- 둘 다 우리가 넣는 "인수" ( useEffect를 호출하는 거니까 - 매개변수 아님! )
- deps 배열 안 값이 바뀌면 → 콜백 실행
- 예 : useEffect(() => console.log(count) , [count]) -> count 바뀔 때마다 콜백함수 실행

### ⭐ deps엔 "값 그 자체"를 넣는다

- ⭕ [count] / ❌ [`count: ${count}`] (우연히되지만 틀린 의도)
- 여러 개: [count, input] → 둘 중 하나라도 바뀌면 실행
- 이름: 이 배열에 의존해서 동작 -> 의존성 배열 (dependency array)

### ⭐ useState vs useEffect — 배열 방향이 반대 ! (헷갈렸던 것)

| 훅        | 배열이 어디           | 방향          |
| --------- | --------------------- | ------------- |
| useState  | 돌려줌(return)        | (나옴 <- )    |
| useEffect | 넣어줌(콜백 , [deps]) | ( 들어감 -> ) |

- console.log(useEffect(...)) → undefined (아무것도 안 돌려줌!)
- 내가 본 배열 = "돌려받은 것"이 아니라 "내가 넣는 것"
- 콜백·deps는 배열의 "요소"가 아니라 "두 개의 인수"

### 어디에 쓰나

- 컴포넌트 안 / 커스텀 훅 안 둘 다 OK (53강 규칙 : 최상위에서만 )
- 단, useEffect가 쓰는 state가 보이는 곳(scope) 안에 있어야 함 (클로저 !)
- 예 : input 이 APP에 있으면 -> useEffect는 input 못 봄

### ⭐⭐ 왜 핸들러에 안 넣고 useEffect?

    const onClickButton = (value) => {
      setCount(count + value);  // "바꿔줘" 예약
      console.log(count);       // ← 아직 옛날 값! 😵
    };

- setCount = 즉시 바꾸기 아니라 "다음 렌더 예약"
- 그래서 바로 다음 줄 count는 아직 리렌더 전 = 옛값
- 타이밍 차이:
  - 이벤트 핸들러 = 변경 "예약 직후" → 옛값
  - useEffect = 리렌더 "끝난 뒤(반영 후)" → 새값 ✅

- 핸들러 = 예약 직후 → 옛값 / useEffect = 반영 후 → 새값
- 순서: 클릭 → setCount(예약) → 리렌더 → paint → useEffect 콜백 (새 count)

### 💡 한 줄 정리

바뀐 state로 뭔가 하고 싶으면 → 핸들러 말고 useEffect(콜백 , [그 state]).

## 섹션8. useEffect로 라이프사이클 제어 ( Mount / Update / Unmount)

### 큰 그림

- 어제 배운 라이프사이클 3형제(Mount/Update/Unmount)를 useEffect로 제어
- useEffect(콜백, deps?) — 2번째 인수(deps)는 생략 가능(optional)

### ⭐ 축 1 — 콜백은 언제? (deps가 결정)

| deps    | 콜백 실행 시점               |
| ------- | ---------------------------- |
| 생략    | mount + 매 렌더(update) 마다 |
| []      | mount 때 딱 1번              |
| [count] | mount + count가 바뀔 때만    |

- deps = "이게 바뀌면 실행해줘" 감시 필터
- 필터 없음 (생략) -> 거를 게 없어서 매번
- 필터 빔([]) -> 감시 대상 없어 mount 후 안 돎
- 필터 값 ([count]) -> 그 값 바뀔 때
- 2번째 인수 생략해도 에러 안남 → undefined 됨 (세션1~2 인자부족)
  -> React : deps 가 undefined 면 매 렌더 실행

### ⭐ 공통 규칙 + 플래그

- deps 뭘 넣든 "mount 때는 콜백 항상 1번" 실행
- "update 때만" 원하면 → 플래그(flag) 로 mount 실행을 걸러냄
- 플래그 = 상태 표시용 true/false 변수 (여기선 마운트 지났나 ? 깃발)
  const isMount = useRef(false); //시작 : 아직 mount 안 지남
  useEffect(() => {
  if (!isMount.current) { // mount: !false=true → 진입
  isMount.current = true; // 깃발 세움
  return; // 강제 종료 → 아래 console 스킵
  }
  console.log("update만!"); // update: !true=fasle -> if 스킵 → console 실행
  });
- mount: !false=true → if 실행 → 깃발↑ + return(스킵)
- update: !true=false → if 건너뜀 → console 실행
- isMount에 useRef 쓰는 이유: 값 바꿔도 리렌더 (X) + 렌더 넘어 유지 (state 면 무한루프)

### ⭐ 축 2 — cleanup은 언제? (콜백 타이밍과 별개 !)

- useEffect 콜백이 return한 함수 = cleanup
- 실행 시점 2가지:
  ① 콜백이 다시 돌기 "직전" (이전 거 치우고 새 거) = "다음 실행 직전"
  ② 컴포넌트 unmount 될 때 (마지막 거 치우기)
- []면 → 다시 돌 일 없음 → cleanup은 오직 unmount 때만
- 타임라인([count]): 실행0 → (정리 0→실행1) → (정리1→실행2) → 정리2(unmount)
- 비유 : 호텔방 . 새 손님 전에 이전 흔적 치움 / 폐업 때도 치움

### ⭐ StrictMode 주의 (dev 전용)

- 개발 모드는 mount 때 일부러 setup→cleanup→setup 돌림(cleanup 점검용)
- → 마운트 직후 "unmount"가 한 번 떠도 버그 아님!
- 진짜 unmount 확인: 조건으로 컴포넌트 사라뜨려 보기 (프로덕션은 헛로그 X)

### 💡 한 줄 정리

콜백 타이밍은 deps가 결정(생략/[]/[값]), cleanup은 별개 축(다음 실행 직전 or unmount).
mount는 항상 실행되니 update만 원하면 플래그로 거른다.

## 섹션10. useReducer — 상태 관리 로직을 컴포넌트 밖으로

### 왜 쓰나?

- state 변경 로직이 길어지면 → 컴포넌트의 진짜 목적( UI 렌더링)이 안 보임 -> 가독성·유지보수 ↓
- useState는 로직 밖으로 못 뺌 -> useReducer는 뺄 수 있음 !
- 모든 useState는 useReducer 로 대체 가능 (사실상 흡사)

### 기본 형태

    const [state, dispatch] = useReducer(reducer, 초기값);

- (useState처럼)배열 반환: [0]=state / [1]= dispatch
- dispatch = "상태 바꿔줘 !" 요청만 하는 함수 (발송 / 급송)
- reducer = 실제로 새 state 만드는 함수 (컴포넌트 "밖"에!)
...
### ⭐ 실행 흐름 4단계
    ...
    ① dispatch({type, data})   ← 방아쇠! 요청 발송
    ② useReducer가 reducer(현재state, 액션객체) 자동 호출
    ③ reducer가 action 보고 → 새 state 계산해서 "return만"
    ④ useReducer가 return값을 → 새 state로 slot 저장 + 리렌더
    - useReducer(reducer , 0) 은 reducer를 바로 호출 X → dispatch 올 때만 호출!

### ⭐ 핵심 = reducer는 "return만" (setState 없음!)

- reducer 안엔 setState가 없다 ! → 그냥 새 state를 return 만 함
- 그 값을 useReducer가 받아서 실제 state로 저장+리렌더
- (요리사는 요리만=return / 서빙은 시스템= useReducer )

### switch 문

    switch (action.type) {     // 이 값을 보고 분기 !
      case "INCREASE": return state + action.data;   // "INCREASE"면
      case "DECREASE": return state - action.data;
      default: return state;      // ★ 모르는 요청도 state 유지 (return 꼭!)    // 아무것도 안 맞으면 (else)
    }

- switch = if-else 여러 개 깔끔하게 (type별 분기)
- return 있으면 그 case 끝 (break 불필요)
- default에 return state ! 안 하면 undefined 됨! (모르는 요청 와도 state 유지, 안 하면 undefined)
